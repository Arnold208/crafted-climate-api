...
async function checkDeviceAccessCompatibility(req, device, requiredPermission = null) {
  ...
  if (device.organizationId && currentOrgId === device.organizationId) {
    ...
    if (req.orgMembership) {
      const permissions = req.orgMembership.permissions || [];
      ...
      if (requiredPermission) {
        ...
        if (permissions.includes("org.devices.view")) {
          return true;
        }
      }
    }
...
function deviceAccessMiddleware(requiredPermission = null) {
  return async (req, res, next) => {
    try {
      ...
      let device = null;
      if (auid) {
        device = await Device.findOne({ auid });
      } else if (deviceId) {
        device = await Device.findOne({ _id: deviceId });
      } else if (devid) {
        device = await Device.findOne({ devid });
      }
