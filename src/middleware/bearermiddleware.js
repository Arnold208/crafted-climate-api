...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err, decoded) => {
    ...
    try {
      ...
      const user = await User.findOne({ userid: decoded.userid }).select('userid deletedAt role platformRole organizations currentOrganizationId email username firstName lastName');
